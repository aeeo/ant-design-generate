...
// uzzuz\\uz-front\\components 组件目录绝对路径
const generateComponentsPath = path.join(absolutePath, "uz-front\\components");
const port = 8081;
...
app.post("/getFilesContent", (req, res) => {
  ...
  filesInfo.css = fs.readFileSync(path.join(absolutePath, req.body.cssPath), "utf-8", function (error, dataStr) {
    if (error) {
      console.error(error);
    }
    return;
...
app.post("/generate", async (req, res) => {
  ...
  // 组件根目录名
  let generateComponentName = body.generateComponentName;
  // 首字母大写文件名
  let generateComponentNameHump = toFirstUpperCase(generateComponentName);
  // 组件绝对路径
  let generateComponentPath = path.join(absolutePath, "uz-front", body.generatePath, generateComponentName);
  // 组件资源相对路径
  let generateComponentAssetsRelativePath = path.join("assets", body.generatePath, generateComponentName);
  // 组件资源绝对路径
  let generateComponentAssetsPath = path.join(absolutePath, "uz-front\\assets", body.generatePath, generateComponentName);
  // uzzuz文件相对与生成组件目录路径
  let uzzuzRelativePath = path.join(path.relative(generateComponentPath, generateComponentsPath), "uzzuz").replace(/\\+/g, "/");

  let unCss = body.unCss;
  let html = body.html;
  let cssFileAbsolutePath = path.join(absolutePath, body.cssPath); // Css文件路径
  let cssPath = path.join(body.cssPath, "../"); // Css文件所在目录
  let htmlFileAbsolutePath = path.join(absolutePath, body.htmlPath); // Html文件路径
  let htmlPath = path.join(body.htmlPath, "../"); // Html文件所在目录
  ...
  // 根目录删除
  fs.rmSync(generateComponentPath, { force: true, recursive: true });
  fs.rmSync(generateComponentAssetsPath, { force: true, recursive: true });
  // [{type:"",path: "",writeStr: ""}]
  let filesInfo = generateFiles(
  ...
  let componentPreviewImage = path.join(generateComponentPath, "demo.png");
  let componentAssetsPreviewImage = path.join(generateComponentAssetsPath, "demo.png");
  await generateComponentImage(htmlFileAbsolutePath, componentPreviewImage, componentAssetsPreviewImage);
  let blockList = [
    {
      blockId: "blockId",
  ...
  let openPreviewUrl = previewUrl + "?type=batch&blockList=" + encodeURI(JSON.stringify(blockList));
  // 打开组件预览
  opn(openPreviewUrl);
  return ResultSuccess(res, "创建成功");
});
...
// 替换Tag
function replaceTag(templateStr, html, componentName, uzzuzRelativePath, privateHtmlImageListField, privateCssImageListField) {
  // 模板信息
...
function extractHtmlImages(html, htmlPath, imagesPath, imagesRelativePath, callback) {
  // 提取src图片
...
function extractCssImages(unCss, cssPath, imagesPath, imagesRelativePath, callback) {
...
// 生成组件图片预览
async function generateComponentImage(htmlFileAbsolutePath, componentPreviewImage, componentAssetsPreviewImage, callback) {
  const timeOut = 5000;
...
// 返回
function ResultSuccess(res, message, data) {
  return res.json({
    successed: true,
    code: 200,
    data: data,
...
// 根据下标替换字符串
function replaceStr(str, replaceStr, replacedStr, num) {
